...
UNIT
systemctl stop nodelume-agent-helper.service nodelume-agent-helper.socket >/dev/null 2>&1||true;systemctl daemon-reload >/dev/null;systemctl enable --now nodelume-agent-helper.socket >/dev/null 2>&1||true
if [ -n "$SERVER" ] || [ -n "$TOKEN" ]; then
